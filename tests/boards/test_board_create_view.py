...
class TestBoardCreateView:
    ...
    def test_auth_required(self, client, board_create_data):
        """
        Тест проверяет: неавторизованного пользователя при создании доски, получит ошибку 403 авторизации.
        """
        response = client.post(self.url, data=board_create_data())

        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_failed_to_create_deleted_board1(self, auth_client, board_create_data):
        """
        Тест проверяет: нельзя создавать удалённую доску.
        """
    ...
    def test_request_user_became_board_owner(self, auth_client, user, board_create_data):
        """
        Тест проверяет: Пользователя создавшего доску на владельца.
        """
